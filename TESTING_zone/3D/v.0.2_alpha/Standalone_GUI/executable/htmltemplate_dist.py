...
def KMZ_THREEJS_ADDONS_FILES_ADD_TO_PROJECT(projectName):
	...
	# create root folder tree if does not exist
	root = 'three/addons/jsm'
	if not os.path.exists(os.path.join(projectName, root)):
		os.makedirs(os.path.join(projectName, root), exist_ok=True)

	# create subfolders if they do not exist
	subflds = ('controls', 'libs', 'loaders')
	for fld in subflds:
		if not os.path.exists(os.path.join(projectName, root, fld)):
			os.mkdir(os.path.join(projectName, root, fld))
	
	# copy the files to the project folder
	js_list = ('controls/OrbitControls.js', 
	...
	for js in js_list:
		subfld, file = os.path.split(js)
		root = os.path.join('three/addons/jsm', subfld)
		if not os.path.exists(os.path.join(projectName, root, file)):
			src = os.path.join(get_cwd(), root, file)
			dst = os.path.join(projectName, root)
			shutil.copy(src, dst)
